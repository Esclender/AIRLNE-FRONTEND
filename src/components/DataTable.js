...
const DataTable = ({ route, headers }) => {
    ...
    return (
      <>
              ...
              headers.map((head, i) => {
                if (i !== 0) {
                  if (new Date(row[head]) != 'Invalid Date' && typeof row[head] != 'number') {
                    return <TableCell style={cStyel} align='right' >{new Date(row[head]).toLocaleDateString()}</TableCell>
                  }

                  return <TableCell style={cStyel} align='right' >{row[head]}</TableCell>
                }
        ...
        <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={6}>
          ...
          {
            isAdmin
              ? <AdminActions row={row} route={route} labels={labels} open={open} headers={headers} instance={server} setRows={setRows} />
              : () => hideIcon(false)
          }
  ...
  return (
    <>
    <TableContainer className="ml-16 mt-8 shadow-lg shadow-gray-700 " style={{ height: '550px', width: '90%', borderRadius: '20px' }}>
      ...
      <Table sx={{ overflow: 'scroll' }} aria-label="simple table">
          ...
          <TableRow>
            ...
            <TableCell style={cellStyle} >{getHeadVal()[0]}</TableCell>
            {
              getHeadVal().map((header, i) => {
                if (i !== 0) {
                  return <TableCell style={cellStyle} align='right' >{header}</TableCell>
                }
                return null
              })
            }
        ...
        <TableBody>
          {
            rows !== undefined
              ? rows.map(row => <Row row={row} headers={getHeadKeys()} labels={getHeadVal()} isAdmin={isAdmin} />)
              : <></>
          }
        </TableBody>
                ...
                <TableFooter sx={stickStyle} >
                  <TableRow>
                    <TableCell >
                      ...
                      <CreateOne row={rows[0]} headers={getHeadKeys()} instance={server} labels={getHeadVal()} setRows={setRows} setModalPre={setModalPre} setModalPretext={setModalPretext} setModalNew={setModalNew} modalNew={modalNew} />
